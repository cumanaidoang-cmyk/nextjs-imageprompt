...
const defaultCatalog: CharacterEntry[] = [
  {
    name: 'Cappuccina',
    desc:
      'Baby doll with a cappuccino cup head; joyful smile; big sparkling eyes; soft pastel pajamas. Skin looks hyper-real with gentle pores and soft blush; plush-toy vibe but lifelike; friendly, huggable proportions.',
  },
  {
    name: 'Tiny Orca',
    desc:
      'Tiny orca mascot at toy scale; glossy smooth body; adorable cheeks; playful fins waving. Hyper-real micro-texture with soft specular highlights; cute baby proportions; gentle, friendly expression.',
  },
  {
    name: 'Tungtung',
    desc:
      'Humanoid wood cylinder mascot; carved natural wood grain; warm brown tones; kind eyes. Silky polished surface with fine engraving; gentle smile; friendly posture; plush-toy presentation but realistic texture.',
  },
  {
    name: 'Tatata',
    desc:
      'Fluffy teapot creature with short soft fur; slightly shy eyes; subtle blush. Round baby proportions; cuddly look; seam details like premium plush; lifelike fur fibers with soft shadowing.',
  },
];
...
function CharacterField({
  ...
  onAdd: (entry: CharacterEntry) => void;
  onEdit: (entry: CharacterEntry) => void;
}) {
  const [draftName, setDraftName] = useState('');
  const [draftDesc, setDraftDesc] = useState('');

  const current = catalog.find((c) => c.name === selectedName);
  ...
  return (
    <div className="space-y-2">
      <label className="block text-sm font-medium text-gray-800">Character</label>
      <select
        className="w-full rounded-xl border border-gray-300 bg-white p-3 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-black/10"
        value={selectedName}
...
export default function PromptFormPage() {
  ...
  const addCharacter = (entry: CharacterEntry) =>
    setState((s) => ({
      ...s,
      catalog: [entry, ...s.catalog.filter((c) => c.name !== entry.name)],
      selectedCharacterName: entry.name,
    }));

  const editCharacter = (entry: CharacterEntry) =>
    setState((s) => ({
      ...s,
      catalog: s.catalog.map((c) => (c.name === entry.name ? entry : c)),
    }));
...
');
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-8">
      ...
      <p className="mt-6 text-xs text-gray-500">Tip: tambahkan versi suara/efek audio di bagian akhir prompt bila perlu.</p>
    </main>
  );
}
